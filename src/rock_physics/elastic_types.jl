...
ρ : density (kg/m³)
"""
mutable struct anharmonic{T1,T2,T3,T4,T5}
    T::T1
    P::T2
    ρ::T3
    Gu_TP::T4
...
function forward(m::anharmonic; params=params_anharmonic.Isaak1992)
    @unpack T_K_ref, P_Pa_ref, Gu_0_ol, dG_dT, dG_dP, ν, Gu_0_crust, dG_dT_crust, dG_dP_crust = params
    ...
    if m.GU_TP < 0 && m.Ku_TP < 0
        ΔT = m.T - T_K_ref # K
        ΔP = m.P * 1.0f9 - P_Pa_ref # Pa
        m.GU_TP = calc_Gu(Gu₀ * 1.0f9, ΔT, ΔP, dG_dT₀, dG_dP₀)
        m.Ku_TP = calc_Ku(m.GU_TP, ν)
    elseif m.GU_TP < 0 && m.Ku_TP > 0
    ...
    Vp = calc_Vp(m.Ku_TP, m.Gu_TP, m.ρ)
    Vs = calc_Vs(m.Gu_TP, m.ρ)

    return RockphyElastic(m.Gu_TP, m.Ku_TP, Vp, Vs)

end

mutable struct anharmonic_poro{T1,T2,T3,T4,T5,T6}
    T::T1
    P::T2
    ρ::T3
    ϕ::T4
...
function melt_bulk_moduli(k, ϕ, A, Km, ν)
    ...
    n_k = a_vec[1] * ψ + a_vec[2] * (1 - ψ) + a_vec[3] * ψ + (1 - ψ)^(inv(5.0f0))
...
function forward(m::anharmonic_poro; params=params_anharmonic_poro)
    ...
    anh_p = forward(
        anharmonic(m.T, m.P, m.ρ, m.Gu_TP, m.Ku_TP); params=params_anharmonic
    )

    Gueff, Γ_G = melt_shear_moduli(anh_p.G, m.ϕ, m_A, ν)
    Kueff, Γ_K = melt_shear_moduli(anh_p.K, m.ϕ, m_A, Km, ν)

    Vp, Vs = Vp_Vs_calc(ϕ, anh_p.G, anh_p.K, Γ_G, Γ_K, ρ, Km) # slightly different here
