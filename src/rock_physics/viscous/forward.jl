function forward(m::HZK2011, p; params = default_params_HZK2011)
    ...
    for mech in keys(mechs)
        sr = sr_flow_law_calculation(m.T, P, m.σ, m.dg, m.ϕ, 0, getfield(x_ϕ_c_vec, mech), getfield(mechs, mech))
        @. ϵ_rate += sr
    end
...
function forward(m::HK2003, p; params = default_params_HK2003)
    ...
    for mech in keys(mechs)
        sr = broadcast((T, P, σ, d, ϕ, fH2O) -> sr_flow_law_calculation_HK2003(T, P * 1f9, σ, d, ϕ, fH2O, getfield(x_ϕ_c_vec, mech), mechs, mech), m.T, P, m.σ, m.dg, m.ϕ, fH2O)
        @. ϵ_rate += sr
    end
