...
"""
function transform_utils(p::Vector{T}, tf::Function, itf::Function, dtf::Function) where T <:Union{Float32, Float64}
    
    return transform_utils{eltype(p)}(p, (x)->tf(x, p), (x)->itf(x, p), (x)->dtf(x, p))
end
...
lin_tf= transform_utils([], (x) -> x, (x) -> x, (x) -> 1.);
