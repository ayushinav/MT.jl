...
function stochastic_inverse(r_obs::resp1, err_resp::resp2, vars, alg_cache::rto_cache;
                            trans_utils::NamedTuple=(m=logg_tf, h=lin_tf)) where {
                                                                                  resp1 <:
    ...
    retcode = inverse!(alg_cache.m₀, r_obs, vars, occam_cache(alg_cache.μgrid); W=W,
    ...
    linsolve_prob = init(lin_prob;
                         assumptions=LinearSolve.OperatorAssumptions(true;
                                                                     condition=LinearSolve.OperatorCondition.WellConditioned))
    ...
    @showprogress for i in 1:(alg_cache.n_samples)
        ...
        for k in fieldnames(typeof(r_obs))
            getfield(pert_resp, k) .= rand(MultivariateNormal(getfield(r_obs, k),
                                                              Diagonal(getfield(err_resp,
                                                                                k))))
        end
        ...
        ret_code = inverse!(copy(alg_cache.m₀), r_obs, vars, # 23 use copy(m₀) because we don't want to perturb m₀
                            occam_cache(alg_cache.μgrid); W=W, χ2=alg_cache.χ2, max_iters=1,
                            response_fields=alg_cache.response_fields,
    ...
    return Turing.Chains(broadcast(trans_utils[:m].itf, m_chains)',
                         [Symbol("ρ[$i]") for i in 1:n])
end
