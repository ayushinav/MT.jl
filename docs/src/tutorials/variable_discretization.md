...
hm = get_kde_image!(ax, mt_chain, modelD; kde_transformation_fn = log10, trans_utils = (; m = pow_tf), grid = (m=collect(-1:0.1:5), z=collect(1:50:2.5e3)), colormap = :thermal, colorrange = (-4, 0.))
Colorbar(fig[1,2], hm, label = "log pdf")

mean_kws = (; color = :blue, linewidth = 2)
std_kws = (; color = :red, linewidth = 2)
get_mean_std_image!(ax, mt_chain, modelD, confidence_interval = 0.9; trans_utils = (; m = pow_tf), mean_kwargs = mean_kws, std_plus_kwargs= std_kws, std_minus_kwargs= std_kws, z_points = collect(1:10:2.5e3))
xlims!(ax, [1e-1, 1e5])
ylims!(ax, [2500, 0])
