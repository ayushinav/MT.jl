...
for i in eachindex(f.content[1:end-1])
    ...
    axislegend(ax_, [lv], ["solidus temperature"], position = :rb, labelsize = 12, framevisible = false)
end
...
m = xfit_premelt(T, P, dg, σ, ϕ, T_solidus, Ch2o)
resp = forward(m, []);
...
f[1,2] = Legend(f, f.content[end-1], "Water conc. (ppm)") 
